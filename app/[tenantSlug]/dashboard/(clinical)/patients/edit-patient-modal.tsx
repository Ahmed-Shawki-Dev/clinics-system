...
export function EditPatientModal({ patient }: { patient: IPatient }) {
  ...
  const [initialChronic, setInitialChronic] =
    useState<Partial<UpdatePatientInput> | null>(null);
    ...
    if (open) {
      const fetchChronicData = async () => {
        ...
        if (res.success && res.data) {
          ...
          if (
            Object.values(chronicData).some((val) => val === true) ||
            res.data.otherNotes
          ) {
            setShowChronic(true);
          }
              ...
              >
                ...
                <div className="space-y-4 border-t pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={showChronic}
                      onCheckedChange={setShowChronic}
                      dir="ltr"
                    />
                  </div>

                  {showChronic && (
                    <div className="bg-muted/50 animate-in fade-in zoom-in-95 grid grid-cols-2 gap-4 rounded-xl p-4 duration-200">
