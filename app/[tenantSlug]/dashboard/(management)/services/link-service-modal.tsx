...
import {
  DoctorLinkInput,
  DoctorLinkSchema,
} from "@/validation/doctor-services";
...
export function LinkServiceModal({
  ...
  const activeCatalogService = catalogServices.find(
    (s) => s.id === selectedServiceId,
  );

  const onSubmit = async (values: DoctorLinkInput) => {
    try {
      const result = await upsertDoctorLinkAction(
        tenantSlug,
        doctorId,
        values.clinicServiceId,
        {
      ...
      if (result.success) {
        toast.success(
          existingLink ? "تم تعديل التسعير بنجاح" : "تم ربط الخدمة بنجاح",
        );
        setOpen(false);
        router.refresh();
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-125" dir="rtl">
        <DialogHeader>
          <DialogTitle>
            {existingLink ? "تعديل تسعير الخدمة" : "ربط خدمة جديدة للطبيب"}
          </DialogTitle>
          <DialogDescription>
            يمكنك تخصيص سعر أو مدة محددة لهذا الطبيب، أو تركها فارغة لاستخدام
            السعر الافتراضي للعيادة.
          </DialogDescription>
        </DialogHeader>
