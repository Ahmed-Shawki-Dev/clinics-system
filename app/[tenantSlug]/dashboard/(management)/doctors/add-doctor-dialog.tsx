  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 2 * 1024 * 1024)
        return toast.error("حجم الصورة يجب أن لا يتعدى 2 ميجا");
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        if (selectedFile) {
          ...
          if (!photoRes.success)
            toast.error("تم إنشاء الدكتور ولكن فشل رفع الصورة");
        }
    ...
    >
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="flex flex-col items-center justify-center gap-2 py-4">
          <div
            className="border-primary/40 bg-muted/50 group hover:border-primary relative flex h-24 w-24 cursor-pointer items-center justify-center overflow-hidden rounded-full border-2 border-dashed shadow-sm transition-all"
