...
export function SettingsForm({ initialData, tenantSlug }: SettingsFormProps) {
  const defaultDays = Object.keys(
    DAYS_AR,
  ) as UpdateSettingsInput["workingHours"][0]["dayOfWeek"][];
  ...
  const form = useForm<UpdateSettingsInput>({
    ...
    defaultValues: {
      ...
      workingHours: defaultDays.map((dayName) => {
        const backendDay = initialData.workingHours?.find(
  ...
  const onSubmit = async (data: UpdateSettingsInput) => {
    try {
      const cleanedSocialLinks = Object.fromEntries(
        Object.entries(data.socialLinks || {}).filter(([, value]) =>
          value?.trim(),
        ),
      );
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Tabs defaultValue="branding" className="w-full">
          <div className="relative w-full overflow-hidden">
            <TabsList className="bg-muted/50 scrollbar-hide mb-8 h-auto w-full flex-nowrap justify-start overflow-x-auto p-1 whitespace-nowrap [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <TabsTrigger
                value="branding"
                className="rounded-md px-4 py-2.5 data-[state=active]:shadow-sm"
              >
                الهوية والمظهر
              </TabsTrigger>
              <TabsTrigger
        ...
        <div className="flex justify-end">
          <Button
            ...
            size="lg"
            disabled={
              form.formState.isSubmitting || isUploadingLogo || isUploadingImage
            }
          >
