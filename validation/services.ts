...
export const ClinicServiceSchema = v.object({
  ...
  defaultPrice: v.pipe(
    v.number("السعر مطلوب"),
    v.minValue(0, "السعر لا يمكن أن يكون بالسالب"),
  ),
  defaultDurationMinutes: v.pipe(
    v.number("المدة مطلوبة"),
    v.minValue(5, "أقل مدة 5 دقائق"),
  ),
...
export type DoctorServiceLinkInput = v.InferOutput<
  typeof DoctorServiceLinkSchema
>;
