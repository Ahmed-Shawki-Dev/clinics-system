...
export async function proxy(request: NextRequest) {
      ...
      const refreshRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/Auth/refresh-token`,
        {
          ...
          headers: {
            ...
            ...(tenantSlug &&
              tenantSlug !== "admin" && { "X-Tenant": tenantSlug }),
          },
  ...
  if (["404", "suspended"].includes(tenantSlug)) {
    ...
    if (staffToken && !response) {
      ...
      if (payload?.tenantSlug && payload.tenantSlug !== tenantSlug) {
        response = NextResponse.redirect(
          new URL(`/${payload.tenantSlug}/dashboard`, request.url),
        );
      }
    }
    ...
    if (patientToken && !response) {
      ...
      if (payload?.tenantSlug && payload.tenantSlug !== tenantSlug) {
        response = NextResponse.redirect(
          new URL(`/${payload.tenantSlug}/patient`, request.url),
        );
      }
    }
    ...
    if (!response) {
      ...
      if (isLandingPage) {
        if (staffToken)
          ...
          response = NextResponse.redirect(
            new URL(`/${tenantSlug}/patient`, request.url),
          );
        else
        ...
        if (patientToken && isAuthPage)
          ...
          response = NextResponse.redirect(
            new URL(`/${tenantSlug}/patient/login`, request.url),
          );
        else
          response = NextResponse.next({
            request: { headers: requestHeaders },
