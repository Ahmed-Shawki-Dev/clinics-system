...
export async function getProfitReportAction(
  ...
  return await fetchApi<IProfitReport>(
    `/api/clinic/finance/profit?${params.toString()}`,
    {
      tenantSlug,
      authType: "staff",
      cache: "no-store",
...
export async function getDailyFinanceAction(tenantSlug: string, date?: string) {
  ...
  return await fetchApi<IDailyFinance>(
    `/api/clinic/finance/daily?${params.toString()}`,
    {
      tenantSlug,
      authType: "staff",
      cache: "no-store",
...
export async function getFinanceByDoctorAction(
  ...
  return await fetchApi<IDoctorProfit[]>(
    `/api/clinic/finance/by-doctor?${params.toString()}`,
    {
      tenantSlug,
      authType: "staff",
      cache: "no-store",
...
// 4. التقرير الشهري
export async function getMonthlyFinanceAction(
  tenantSlug: string,
  year: number,
  month: number,
) {
...
export async function getYearlyFinanceAction(tenantSlug: string, year: number) {
  return await fetchApi<IYearlyFinance>(
    `/api/clinic/finance/yearly?year=${year}`,
    {
      tenantSlug,
      authType: "staff",
      cache: "no-store",
