...
export async function getSubscriptions(
  ...
  const query = tenantId
    ? `?tenantId=${tenantId}&pageSize=1000`
    : "?pageSize=1000";
  return await fetchApi<IPaginatedData<ISubscription>>(
    `/api/platform/subscriptions${query}`,
  );
}
...
export async function extendSubscriptionAction(
  ...
  const res = await fetchApi<ISubscription>(
...
export async function cancelSubscriptionAction(
  ...
  const res = await fetchApi<ISubscription>(
    `/api/platform/subscriptions/${id}/cancel`,
    {
      method: "POST",
      body: JSON.stringify(data),
    },
...
export async function markAsPaidAction(
  ...
  const res = await fetchApi<ISubscription>(
    `/api/platform/subscriptions/${id}/mark-paid`,
    {
      method: "POST",
      body: JSON.stringify(data),
    },
