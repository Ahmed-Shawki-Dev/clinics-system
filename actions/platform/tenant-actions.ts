...
export async function activateTenantAction(
  ...
  const res = await fetchApi<ITenant>(`/api/platform/tenants/${id}/activate`, {
    method: "POST",
  });
...
export async function suspendTenantAction(
  ...
  const res = await fetchApi<ITenant>(`/api/platform/tenants/${id}/suspend`, {
    method: "POST",
  });
  if (res.success) revalidatePath("/admin/tenants");
  return res;
...
export async function blockTenantAction(
  ...
  const res = await fetchApi<ITenant>(`/api/platform/tenants/${id}/block`, {
    method: "POST",
  });
  if (res.success) revalidatePath("/admin/tenants");
  return res;
