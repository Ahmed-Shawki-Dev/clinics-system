...
export function PeriodFilter() {
  ...
  const [customTo, setCustomTo] = useState<Date | undefined>(
    parseSafeDate(searchParams.get("to")),
  );
  ...
  return (
    <div className="relative z-10 flex w-full flex-wrap items-center gap-3 sm:w-auto">
      <Select value={range} onValueChange={handlePresetChange} dir="rtl">
        <SelectTrigger className="bg-background h-10 w-full shrink-0 sm:w-40">
      ...
      {range === "custom" && (
        <div className="animate-in fade-in slide-in-from-right-2 flex w-full flex-wrap items-center gap-2 duration-300 sm:w-auto sm:flex-nowrap">
          {/* ========== DatePicker: من تاريخ ========== */}
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant={"outline"}
                className={cn(
                  "h-10 w-full justify-start text-right font-normal sm:w-35",
