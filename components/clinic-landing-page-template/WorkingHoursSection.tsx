"use client";

import { fadeInUp, staggerContainer } from "@/animation";
import { Badge } from "@/components/ui/badge";
import { Typography } from "@/components/ui/typography";
import { DAY_ORDER, DAYS_AR, IPublicWorkingHour } from "@/types/public";
import { formatTime } from "../../lib/formatTime";
import { cn } from "@/lib/utils";
import { motion } from "framer-motion";

export default function WorkingHoursSection({
  workingHours,
}: {
  workingHours: IPublicWorkingHour[];
}) {
  if (!workingHours?.length) return null;

  const sortedHours = [...workingHours].sort(
    (a, b) => (DAY_ORDER[a.dayOfWeek] ?? 99) - (DAY_ORDER[b.dayOfWeek] ?? 99),
  );

  return (
    <section className="bg-background relative overflow-hidden py-15" dir="rtl">
      <motion.div
        className="relative z-10 container mx-auto max-w-5xl px-4 md:px-6"
        variants={staggerContainer}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, margin: "-100px" }}
      >
        {/* العناوين */}
        <div className="mb-14 flex flex-col items-center space-y-4 text-center">
          <motion.div variants={fadeInUp}>
            <Typography
              variant="h2"
              className="text-foreground w-full text-3xl font-bold tracking-tight md:text-5xl"
            >
              مواعيد <span className="text-primary">العيادة</span>
            </Typography>
          </motion.div>
          <motion.div variants={fadeInUp}>
            <Typography variant="p" className="text-muted-foreground text-lg">
              نحن هنا لخدمتك في الأوقات التالية
            </Typography>
          </motion.div>
        </div>

        {/* قائمة المواعيد (بدون ستايل الجدول) */}
        <div className="flex flex-col gap-3">
          {sortedHours.map((wh, index) => {
            const isActive = wh.isActive;
            const dayName = DAYS_AR[wh.dayOfWeek] ?? wh.dayOfWeek;

            return (
              <motion.div
                key={wh.dayOfWeek}
                variants={fadeInUp}
                custom={index} // عشان نعمل تأخير متدرج في الأنيميشن لو حابب
                className={cn(
                  // 🔴 كبسولة منفصلة بدون بوردر، بتعتمد على لون خلفية خفيف جداً
                  "group hover:bg-muted/50 flex flex-row items-center justify-between gap-4 rounded-2xl px-6 py-4 transition-colors",
                  isActive ? "bg-muted/20" : "bg-transparent opacity-70",
                )}
              >
                <div className="flex items-center gap-4">
                  <span
                    className={cn(
                      "text-base font-semibold tracking-wide md:text-lg",
                      isActive ? "text-foreground" : "text-muted-foreground",
                    )}
                  >
                    {dayName}
                  </span>

                  {!isActive && (
                    <Badge
                      variant="secondary"
                      className="bg-muted text-muted-foreground text-xs font-medium"
                    >
                      مغلق
                    </Badge>
                  )}
                </div>

                {isActive ? (
                  <div className="text-foreground flex items-center gap-2 text-sm font-medium md:text-base">
                    <span>{formatTime(wh.startTime)}</span>
                    <span className="text-muted-foreground/50 mx-1">-</span>
                    <span>{formatTime(wh.endTime)}</span>
                  </div>
                ) : (
                  <div className="text-muted-foreground text-sm font-medium">
                    عطلة أسبوعية
                  </div>
                )}
              </motion.div>
            );
          })}
        </div>

        {/* الملاحظة السفلية */}
        <motion.div variants={fadeInUp}>
          <p className="text-muted-foreground mt-8 text-center text-sm font-medium">
            المواعيد قابلة للتعديل في الأعياد والمناسبات الرسمية. يُفضَّل الحجز
            المسبق.
          </p>
        </motion.div>
      </motion.div>
    </section>
  );
}
