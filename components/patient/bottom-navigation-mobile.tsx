...
export function PatientBottomNav() {
  ...
  const navItems = [
    ...
    {
      name: "حجوزاتي",
      href: `/${tenantSlug}/patient/bookings`,
      icon: CalendarDays,
    },
  ...
  return (
    <nav className="bg-background pb-safe dir-rtl fixed right-0 bottom-0 left-0 z-50 flex h-16 w-full items-center justify-around border-t shadow-[0_-4px_6px_-1px_rgb(0,0,0,0.05)]">
      {navItems.map((item) => {
        ...
        return (
          <Link
            ...
            href={item.href}
            className={cn(
              "flex w-16 flex-col items-center justify-center gap-1.5 transition-all duration-200",
              isActive
