...
export function AppSidebar() {
  ...
  const getFullUrl = (href: string) =>
    `/${tenantSlug}/dashboard${href === "/" ? "" : href}`;
  ...
  return (
    <Sidebar collapsible="icon" side="right">
      <SidebarHeader className="flex h-16 shrink-0 flex-row items-center overflow-hidden border-b px-4 group-data-[collapsible=icon]:px-0">
        {tenantConfig && isMounted ? (
          <div className="flex w-full items-center gap-2 group-data-[collapsible=icon]:justify-center">
            <div className="bg-background relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-md">
              <ClinicImage
        ...
        {isLoading ? (
          ...
          filteredConfig.map((category) => (
            <SidebarGroup key={category.label}>
              <SidebarGroupLabel className="text-muted-foreground/70 text-xs font-bold tracking-widest uppercase group-data-[collapsible=icon]:hidden">
                {category.label}
              </SidebarGroupLabel>
                    ...
                    return (
                      <SidebarMenuItem key={item.title}>
                        <SidebarMenuButton
                          ...
                          isActive={isActive}
                          tooltip={item.title}
                          className="hover:bg-primary/5 data-[active=true]:bg-primary/10 transition-all duration-200"
                        >
                          <Link
                            href={fullUrl}
                            onClick={() => {
