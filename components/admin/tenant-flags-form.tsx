...
export function TenantFlagsForm({
  tenantId,
  initialData,
}: TenantFlagsFormProps) {
  ...
  const flags = [
    {
      name: "onlineBooking",
      label: "الحجز أونلاين",
      desc: "تفعيل حجز المواعيد عبر الإنترنت",
    },
    {
      name: "whatsappAutomation",
      label: "أتمتة الواتساب",
      desc: "إرسال رسائل تذكير تلقائية",
    },
    {
      name: "pwaNotifications",
      label: "تنبيهات PWA",
      desc: "تفعيل إشعارات التطبيق على الموبايل",
    },
    {
      name: "expensesModule",
      label: "موديول المصاريف",
      desc: "إدارة الحسابات والمصاريف الداخلية",
    },
    {
    ...
    },
    {
      name: "ratings",
      label: "التقييمات",
      desc: "السماح للمرضى بتقييم الخدمة",
    },
    {
      name: "export",
      label: "تصدير البيانات",
      desc: "إمكانية سحب البيانات Excel/PDF",
    },
  ] as const;
          ...
          {flags.map((flag, index) => (
            <div key={flag.name}>
              <FormField
                ...
                name={flag.name}
                render={({ field }) => (
                  <FormItem className="bg-card flex flex-row items-center justify-between rounded-lg border p-4 shadow-sm">
