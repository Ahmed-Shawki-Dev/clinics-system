...
export function PatientSearch({ tenantSlug, onSelect, selectedPatientId }: PatientSearchProps) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        // w-[--radix-popover-trigger-width] بتخليه نفس عرض الزرار بالظبط
              ...
              {!isLoading &&
                patients.map((patient) => (
                  <CommandItem
                    ...
                    // أضفنا min-w-0 هنا لضمان عمل الـ truncate
                    className='flex items-center justify-between cursor-pointer py-2.5 min-w-0 gap-2'
                  >
                    <div className='flex flex-col flex-1 min-w-0'>
                      {/* truncate هتقص الاسم وتضيف "..." لو طويل جداً */}
                      <span className='font-bold text-sm truncate text-right'>{patient.name}</span>
                      <span
